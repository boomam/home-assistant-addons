# Change Log
## Updates
Tailscale version should automatically update through a GitHub Action.
Base Image will be a manual process for the time being.

## Versioning
First three digits are Tailscale's version number.  
The letter & number are bug fix releases where said issue is not with Tailscale, but with this template.  

-----
# Tailscale
## Tailscale 1.102.4
* Updated Tailscale from 1.102.3 to 1.102.4

## Tailscale 1.102.3
* Updated Tailscale from 1.102.2 to 1.102.3

## Tailscale 1.102.2
* Updated Tailscale from 1.98.10 to 1.102.2

## Tailscale 1.98.10
* Updated Tailscale from 1.98.9 to 1.98.10

## Tailscale 1.98.9
* Updated Tailscale from 1.98.8 to 1.98.9

## Tailscale 1.98.8
* Updated Tailscale from 1.98.4 to 1.98.8

## Tailscale 1.98.4
* Updated Tailscale from 1.98.3 to 1.98.4

## Tailscale 1.98.3
* Updated Tailscale from 1.98.2 to 1.98.3

## Tailscale 1.98.2
* Updated Tailscale from 1.96.4 to 1.98.2


## Tailscale 1.96.4.a
* Updated base image from 20.0.1 to 20.1.0.

## Tailscale 1.96.4
* Updated Tailscale from 1.96.2 to 1.96.4

## Tailscale 1.96.2.a
* Updated base image from v19.0.0 to v20.0.1
* Reverted versioning in build process as TS havn't deployed latest tarballs to their package repo yet.

## Tailscale 1.96.3
* Updated Tailscale from 1.94.2 to 1.96.3

## Tailscale 1.94.2
* Updated Tailscale from 1.94.1 to 1.94.2

## Tailscale 1.94.1
* Updated Tailscale from 1.92.5 to 1.94.1

## Tailscale 1.92.5
* Updated Tailscale from 1.92.3 to 1.92.5


## Tailscale 1.92.3.a
* Fixed docker health check

## Tailscale 1.92.3
* Updated Tailscale from 1.92.1 to 1.92.3

## Tailscale Beta 2
* Added auto-update via GitHub actions.

## Tailscale Beta 1
* Corrected startup scripts
* Corrected nginx scripts
* Corrected s6-overlay scripts
